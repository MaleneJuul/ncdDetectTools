...
ncdDetectOverdispersion <- function(predictions, scores, overdispersion = 0, N = 10, method = c("naive", "is", "numeric"), observed_score, return_dist = F) {
  ...
  for (i in 1:N) {
    ...
    # - print progress
    cat(paste("calculating p-values using overdispersion. Iteration ", i, " of ", N, ".\n", sep = ""))
    
    # - randomly draw random effect
    ...
    # -perform convolution
    conv_results[[i]] <- convolution(dat, threshold = max(observed_score))
  ...
  score_dist <- rbindlist(conv_results)[, .("probability" = sum(probability)/N), by = y]
